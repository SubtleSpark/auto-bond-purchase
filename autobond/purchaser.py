...
class EastmoneyPurchaser:
    ...
    def run_for_user(self, user: UserCredential, label: str) -> str:
        ...
        for attempt in range(1, self.config.flow_retries + 1):
            ...
            try:
                print(f"[{label}] 开始执行，第 {attempt}/{self.config.flow_retries} 次")
                return self._run_once(page, user)
            except Exception as exc:
                last_error = exc
                self._save_error_screenshot(page, label, attempt)
                error = redact_secrets(str(exc), (user.account, user.password, self.config.pushplus_token))
                print(f"[{label}] 第 {attempt} 次失败: {error}")
            finally:
                context.close()

        if last_error is None:
            raise RuntimeError("未知错误")
        error = redact_secrets(str(last_error), (user.account, user.password, self.config.pushplus_token))
        ...
        for attempt in range(1, self.config.captcha_retries + 1):
            try:
                ...
                if re.fullmatch(r"\d{4}", code):
                    print(f"验证码识别成功 (第{attempt}次)")
                    return code
                raise ValueError("识别结果格式异常")
            except Exception as exc:
                print(f"验证码识别失败 (第{attempt}次): {type(exc).__name__}")
    ...
    def _save_error_screenshot(self, page: Page, label: str, attempt: int) -> None:
        screenshot_dir = Path(self.config.screenshot_dir)
        screenshot_dir.mkdir(parents=True, exist_ok=True)

        suffix = re.sub(r"[^0-9A-Za-z_-]", "_", label)
