...
def parse_users(users_str: str) -> list[UserCredential]:
    ...
    users: list[UserCredential] = []
    for index, pair in enumerate(users_str.split(","), start=1):
        pair = pair.strip()
        if not pair:
            continue
        if ":" not in pair:
            raise ValueError(f"第 {index} 个用户格式错误，应为 账号:密码")
        account, password = pair.split(":", 1)
        account = account.strip()
        password = password.strip()
        if not account or not password:
            raise ValueError(f"第 {index} 个用户的账号或密码为空")
        users.append(UserCredential(account=account, password=password))
